        ...
        <div class="w-1/4">
            ...
            <div class="mt-10 px-3 py-10 bg-white w-5/6 rounded-lg">
                <ul class="space-y-4 flex flex-col items-center">
                    <li class="w-11/12 py-3 transition-all ease-in-out bg-slate-900 bg-opacity-10  rounded-full">
                        <form action="{{ url('userProfile') }}" method="POST">
                            @csrf
                            <button type="submit" class="px-6 block transition-all ease-in-out text-gray-600">
                                Personal Information</button>
                        </form>
                    </li>

                    <li
                        class="w-11/12 py-3 transition-all ease-in-out hover:bg-slate-900 hover:bg-opacity-10  rounded-full">
                        <form action="{{ url('userProfile') }}" method="POST">
                            @csrf
                            <button type="submit" class="px-6 block transition-all ease-in-out text-gray-600">Booking
                                History</button>
                        </form>
                    </li>
                    ...
                    <li
                        ...
                        <form action="{{ url('userProfile') }}" method="POST">
                            @csrf
                            <button type="submit"
                                class="px-6 block transition-all ease-in-out text-gray-600">Tournament</button>
                        </form>
                    </li>
                </ul>
            </div>
